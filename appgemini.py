...
if "viral_title" not in st.session_state:
    st.session_state.viral_title = ""
if "viral_tags" not in st.session_state:
    st.session_state.viral_tags = ""
if "thumb_path_A" not in st.session_state:
    st.session_state.thumb_path_A = None
if "thumb_path_B" not in st.session_state:
...
if "sync_offset" not in st.session_state:
...
def sync_by_character_mapping(clean_script, audio_duration, words_per_chunk=8, min_chunk_duration=1.2, offset=0.0):
    ...
    Works better for Burmese since word boundaries are ambiguous.
    """
...
def sync_with_whisper_word_timestamps(clean_script, whisper_data, audio_duration, words_per_chunk=8, min_chunk_duration=1.2, offset=0.0):
    ...
    # Try multiple ways to access word data
...
def smart_sync_pipeline(clean_script, audio_path, whisper_data=None, audio_duration=None, sync_offset=0.0, short_punchy=False):
    ...
    if whisper_data is not None:
        try:
            ...
            st.warning(f"Whisper sync fallback: {str(e)[:100]}")
...
async def generate_tts(text, voice_model, output_file, engine="Edge-TTS", ttsmaker_key="", eleven_key="", custom_eleven_id="", gemini_key="", pitch=0, voice_fx="None"):
    ...
    for i, chunk_text in enumerate(chunks):
        ...
        if "Synergy" in engine:
            ...
            res = requests.post(
                ...
                json={"text": chunk_text, "model_id": "eleven_multilingual_v2"},
    ...
    if needs_ffmpeg:
        ...
        elif "Deep & Energetic (Motivation)" in voice_fx:
            audio = audio.filter('bass', g=10, f=150).filter('treble', g=5, f=3000).filter('volume', 1.5)
        elif "Deep & Chilling (Horror)" in voice_fx:
...
with st.sidebar:
    ...
    # 👇 SYNC OFFSET SLIDER - Global Sync Control
    ...
    st.session_state.sync_offset = st.slider(
        ...
        value=0.0,
        step=0.1,
        help="စာတန်းထိုး စောလွန်း/နောက်ကျလွန်းရင် ညှိပါ။ (-) ဆိုရင် စာတန်းထိုး စောမယ်။ (+) ဆိုရင် နောက်ကျမယ်။"
    )
    ...
    if st.button("Save Current Project"):
        proj_data = {
            ...
            "sync_offset": st.session_state.sync_offset
        }
        ...
        if not api_key_input:
            ...
            with st.spinner(f"⏳ [အဆင့်၂/၆] {ai_provider} ဖြင့် ဇာတ်ညွှန်း၊ Title နှင့် Thumbnail ထုတ်လုပ်နေပါသည်..."):
                ...
                try:
                    extra_rules = ""
                    if script_hook:
                        extra_rules += " [HOOK]: Start with an extremely engaging 3-second viral hook."
                    if "Slang" in script_style:
                        ...
                        extra_rules += " [SUSPENSE]: Make the storytelling dramatic, fast-paced, and full of suspense."
                    if script_curiosity:
                        extra_rules += " [CURIOSITY]: Insert curiosity gaps in the middle to retain audience attention."
                    if script_tone:
                        extra_rules += " [TONE]: Inject strong emotions and character tones matching the scene."
                    if script_cta:
                        extra_rules += " [CTA]: End the script with a strong Call to Action asking a question."
                    ...
                    if "Gemini" in ai_provider:
                        ...
                        comp = client.chat.completions.create(
                            model="llama-3.3-70b-versatile" if "Groq" in ai_provider else ("gpt-5.5-pro" if "5.5" in ai_provider else "gpt-4o"),
                    ...
                    if "-->" in clean_raw_srt:
                        ...
                        if st.session_state.sync_offset != 0:
                            parsed_timestamps = [
                                (max(0, s + st.session_state.sync_offset), 
                                 max(0.8, e + st.session_state.sync_offset), t) 
                        ...
                        sync_srt, sync_parsed = smart_sync_pipeline(
                            script_for_sync, 
                            a_extracted,
                            whisper_data=st.session_state.whisper_data,
                            audio_duration=audio_dur,
                            sync_offset=st.session_state.sync_offset,
                            short_punchy=sub_short
                        )
                        with open(srt_final, "w", encoding="utf-8-sig") as f:
                            f.write(sync_srt)
            ...
            with st.spinner(f"⏳ [အဆင့်၄/၆] AI Voice Over ထုတ်လုပ်နေပါသည်..."):
                ...
                try:
                    asyncio.run(generate_tts(
                        speech_text if 'speech_text' in locals() and speech_text else strip_audio_tags(st.session_state.generated_script),
                        voice_char, a_generated,
                        engine=audio_engine_choice,
                        ttsmaker_key=key_ttsmaker,
                        eleven_key=locals().get('eleven_key_input', ''),
                        custom_eleven_id=locals().get('custom_eleven_id', ''),
                        gemini_key=locals().get('synergy_key', api_key_input),
                        pitch=pitch_level,
                        voice_fx=fx_level
                    ))
                except Exception as e:
            ...
            with st.spinner("⏳ [အဆင့်၅/၆] ဗီဒီယိုနှင့် စာတန်းထိုးပေါင်းစပ်နေပါသည်..."):
                ...
                success, err_msg = render_premium_saas_video(
                    ...
                    cb_bypass, cb_blur, watermark_text, subtitle_mode,
                    cb_mirror, cb_color, cb_grain, cb_fps,
                    sub_position=sub_position, sub_color=sub_color,
                    sub_size=sub_size, sub_thickness=sub_thickness,
                    sub_bg=sub_bg, use_freeze=cb_freeze,
                    logo_path=locals().get('uploaded_logo', None),
                    ...
                    if os.path.exists(selected_bgm_path):
                        try:
                            ducked = ffmpeg.filter([
                                ffmpeg.input(selected_bgm_path, stream_loop=-1).audio.filter('aresample', 44100).filter('volume', bgm_volume),
                                ffmpeg.input(v_final).audio
                            ], 'sidechaincompress', threshold=0.04, ratio=4, attack=50, release=300)
            ...
            with st.spinner("⏳ [အဆင့်၂/၅] AI သရုပ်ဆောင်ဖြင့် အသံဖန်တီးနေပါသည်..."):
                ...
                try:
                    ...
                    asyncio.run(generate_tts(
                        ...
                        fc_voice_char, "fc_audio.wav",
                        engine=fc_audio_engine,
            ...
            with st.spinner("⏳ [အဆင့်၄/၅] စာတန်းထိုးများကို ချိန်ညှိနေပါသည်..."):
                ...
                try:
                    ...
                    sync_srt, fc_parsed = smart_sync_pipeline(
                        clean_script_for_sync,
                        "fc_audio.wav",
                        whisper_data=st.session_state.whisper_data,
                        audio_duration=fc_audio_dur,
                        sync_offset=st.session_state.sync_offset,
                        short_punchy=fc_sub_short
                    )
                    with open("subtitles.srt", "w", encoding="utf-8-sig") as f:
                        f.write(sync_srt)
                    
                    if not fc_parsed or len(fc_parsed) == 0:
                        last_err = "No parsed timestamps generated"
                        fc_parsed = None
            ...
            with st.spinner("⏳ [အဆင့်၅/၅] အားလုံးကိုပေါင်းစပ်ပြီး Master Video ထုတ်လုပ်နေပါသည်..."):
                ...
                try:
                    success, err_msg = render_premium_saas_video(
                        ...
                        use_bypass=True, subtitle_mode=fc_subtitle_mode,
                        sub_position=fc_sub_position, sub_color=fc_sub_color,
                        sub_size=fc_sub_size, sub_thickness=2.5, sub_bg=False,
                    ...
                    if fc_bgm not in ["None (BGM မထည့်ပါ)"]:
                        ...
                        if os.path.exists(bgm_path):
                            try:
                                ducked = ffmpeg.filter([
                                    ffmpeg.input(bgm_path, stream_loop=-1).audio.filter('aresample', 44100).filter('volume', fc_bgm_vol),
                                    ffmpeg.input(v_final).audio
                                ], 'sidechaincompress', threshold=0.04, ratio=4, attack=50, release=300)
                                mixed = ffmpeg.filter([ffmpeg.input(v_final).audio, ducked], 'amix', inputs=2, duration='first').filter('volume', 2.0)
...
# =====================================================================
    ...
    if st.button("⬇️ ဗီဒီယိုစစ်ဆေးပြီး ဒေါင်းလုဒ်ဆွဲမည်"):
        pass
