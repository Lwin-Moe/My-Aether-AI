# =====================================================================
# 📌 AETHER FILMWORKS AI // STUDIO V52 (HARDENED RENDER PIPELINE)
# =====================================================================
...
# 👇 FIX: Hardened Render Pipeline to prevent FFmpeg sync/crash errors
...
[TAGS: #tag1 #tag2]"""
            ...
            with st.spinner("⏳ [အဆင့်၃/၅] Visuals များကို ပြင်ဆင်နေပါသည်..."):
                ...
                try:
                    ...
                    # 👇 FIX: Apply dynamic scaling accurately during generation
                    ...
                    if "Upload" in fc_visual_mode:
                        ...
                        for i, img_file in enumerate(fc_uploaded_images):
                            ...
                            # Explicitly force exact output size to prevent concat crash later
                            subprocess.run([FFMPEG_BINARY, "-y", "-loop", "1", "-framerate", "25", "-i", img_path, "-t", str(clip_dur), "-vf", f"scale=-2:2000,zoompan=z='min(zoom+0.001,1.15)':d={int(clip_dur*25)}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s={v_w}x{v_h},fps=25", "-c:v", "libx264", "-pix_fmt", "yuv420p", "-preset", "superfast", clip_path], capture_output=True)
...
Format strictly separated by a pipe '|'. Story: {fc_story_text[:300]}"""
                    ...
                    # 👇 FIX: Ensure stable concatenation by removing potentially conflicting formatting args
            ...
            with st.spinner("⏳ [အဆင့်၅/၅] အားလုံးကိုပေါင်းစပ်ပြီး Master Video ထုတ်လုပ်နေပါသည်..."):
                ...
                try:
                    # 👇 Strict checking to halt program on exact failures
