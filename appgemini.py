...
# --- 1. THEME & STYLING ---
st.set_page_config(page_title="AETHER FILMWORKS AI // STUDIO V42", layout="wide")
...
async def generate_tts(text, voice_model, output_file, pitch_val=0, speed_val=0, engine="Edge-TTS (Default Free)", ttsmaker_key="", eleven_key="", custom_eleven_id="", gemini_key=""):
    if "Synergy" in engine:
        ...
        # 🔥 FIX: Added SafetySettings to BLOCK_NONE to prevent the 'parts' error 🔥
        payload = {
...
# --- 3. UI INTERFACE ---
st.markdown('<h1 style="text-align:center; margin-bottom: 30px;">▲ AETHER FILMWORKS AI // MULTI-CORE V42</h1>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if "Gemini" in ai_provider:
        api_key_input = st.text_input("Gemini Keys (Comma separated for Auto-Switch)", type="password", value=saved_gemini, placeholder="Key1, Key2, Key3...")
        if api_key_input and api_key_input != saved_gemini: save_key(API_KEY_FILE, api_key_input)
        st.caption("💡 Quota ကုန်ပါက နောက် Key သို့ အော်တိုပြောင်းပေးပါမည်။")
    ...
    if not api_key_input: st.error("⚠️ API Key အား ထည့်သွင်းပေးပါ ဆရာကြီး။")
        ...
        with st.spinner(f"⏳ [အဆင့် ၄/၆] {audio_engine_choice} စနစ်ဖြင့် AI Voice Over ထုတ်လုပ်နေပါသည်..."):
            try:
                ...
                final_gemini_key = locals().get('synergy_key', '')
...
# --- 5. DISPLAY SECTION (DUAL COLUMN LAYOUT WITH MINI WINDOWS) ---
